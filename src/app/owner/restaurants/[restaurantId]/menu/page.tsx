...
export default async function MenuPage({ params }: Props) { const user = await requireOwnerPageUser(); const { restaurantId } = await params; const restaurant = await prisma.restaurant.findUnique({ where: { id: restaurantId }, include: { menuCategories: { where: { isActive: true }, orderBy: { sortOrder: "asc" }, include: { items: { where: { isAvailable: true }, orderBy: { sortOrder: "asc" } } } } } }); if (!restaurant || !canManageRestaurant(user, restaurant)) notFound(); return <div className="page owner-layout"><div className="page-title"><p className="eyebrow">Настройки ресторана</p><h1>{restaurant.title}</h1></div><OwnerTabs restaurantId={restaurant.id} /><MenuManager restaurantId={restaurant.id} categories={restaurant.menuCategories} /></div>; }
