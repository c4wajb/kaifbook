...
export async function POST(request: Request, context: C) { try { const { restaurantId } = await context.params; const user = await getCurrentUser(); if (!user) throw new ApiError(401, "Authentication required"); await requireRestaurantAccess(user, restaurantId); const payload = menuCategorySchema.parse(await readJson(request)); const category = await prisma.menuCategory.create({ data: { restaurantId, ...payload } }); return ok({ category }, 201); } catch (error) { return handleApiError(error); } }
