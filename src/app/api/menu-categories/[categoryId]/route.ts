...
export async function PUT(request: Request, context: C) { try { const { categoryId } = await context.params; const user = await getCurrentUser(); if (!user) throw new ApiError(401, "Authentication required"); await requireMenuCategoryAccess(user, categoryId); const payload = menuCategorySchema.partial().parse(await readJson(request)); const category = await prisma.menuCategory.update({ where: { id: categoryId }, data: payload }); revalidateTag("restaurant-menu", { expire: 0 }); return ok({ category }); } catch (error) { return handleApiError(error); } }
export async function DELETE(_request: Request, context: C) { try { const { categoryId } = await context.params; const user = await getCurrentUser(); if (!user) throw new ApiError(401, "Authentication required"); await requireMenuCategoryAccess(user, categoryId); const category = await prisma.menuCategory.update({ where: { id: categoryId }, data: { isActive: false } }); revalidateTag("restaurant-menu", { expire: 0 }); return ok({ category }); } catch (error) { return handleApiError(error); } }
