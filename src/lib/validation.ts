...
export const bookingPricingRuleSchema = z.object({ title: z.string().trim().min(2), tableTypeId: z.string().optional().nullable(), tableId: z.string().optional().nullable(), minGuests: z.coerce.number().int().positive().optional().nullable(), maxGuests: z.coerce.number().int().positive().optional().nullable(), dayOfWeek: z.coerce.number().int().min(0).max(6).optional().nullable(), startTime: timeSchema.optional().nullable(), endTime: timeSchema.optional().nullable(), depositAmount: z.coerce.number().int().nonnegative().default(0), bookingPrice: z.coerce.number().int().nonnegative().default(0), isDepositRequired: z.coerce.boolean().default(false), priority: z.coerce.number().int().default(0), isActive: z.coerce.boolean().default(true) });
